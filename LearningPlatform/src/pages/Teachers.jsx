...
//                   className="px-4 py-2 btn btn-outline-light"
//                 >
//                   Cancel
//                 </button>
//                 <button
      ...
      <div className="container">
          ...
          {selectedTeacher && (
            <form className="needs-validation" noValidate>
              ...
              <div className="mb-3">
                ...
                <input type="number" value={formData.price_per_call} onChange={(e) => setFormData({ ...formData, price_per_call: e.target.value })} className="form-control bg-dark text-white" min="0" />
              </div>
              <div className="mb-3">
                <label className="form-label">Experience (Years)</label>
                <input type="number" value={formData.experience_years} onChange={(e) => setFormData({ ...formData, experience_years: e.target.value })} className="form-control bg-dark text-white" min="0" />
              </div>
              <div className="mb-3">
                <label className="form-label">Bio</label>
                <textarea value={formData.bio || ''} onChange={(e) => setFormData({ ...formData, bio: e.target.value })} rows={4} className="form-control bg-dark text-white" placeholder="Enter bio" />
              </div>
              <div className="d-flex justify-content-end gap-2 pt-3">
                <button type="button" onClick={() => setModalType(null)} className="btn btn-outline-light">Cancel</button>
                <button type="button" onClick={handleSaveUpdate} className="btn btn-primary">Save Changes</button>
              </div>
            </form>
          )}
        </Modal>
        ...
        <Modal isOpen={modalType === 'preview'} onClose={() => setModalType(null)} title="Instructor Preview" size="md">
          {selectedTeacher && (
            <div className="row g-3">
              ...
              <div className="col-8"><strong>Status:</strong> {selectedTeacher.profile?.email_verified ? 'Verified' : 'Pending'}</div>
              {selectedTeacher.bio && <div className="col-12"><strong>Bio:</strong> {selectedTeacher.bio}</div>}
            </div>
          )}
        </Modal>
        ...
        <Modal isOpen={modalType === 'wallet'} onClose={() => setModalType(null)} title={`Wallet - ${selectedTeacher?.profile?.full_name || 'Instructor'}`} size="lg">
          {selectedTeacher && (
            <div>
              {walletLoading ? (
                <p className="text-center py-4">Loading wallet...</p>
              ) : (
                <>
                  {walletData && (
                    <div className="row mb-3">
                      <div className="col-6">
                        <p><small>Total Balance (₹)</small></p>
                        <p className="fw-bold">₹{Number(walletData.total_balance ?? 0).toLocaleString()}</p>
                      </div>
                      <div className="col-6">
                        <p><small>Available Balance (₹)</small></p>
                        <p className="fw-bold">₹{Number(walletData.available_balance ?? 0).toLocaleString()}</p>
                      </div>
                    </div>
                  )}
                  <div className="row g-3 mb-3">
                    <div className="col-6">
                      ...
                      <input type="number" min="0" step="1" value={walletForm.total_balance} onChange={(e) => setWalletForm({ ...walletForm, total_balance: e.target.value })} className="form-control" />
                    </div>
                    <div className="col-6">
                      <label>Available Balance (₹)</label>
                      <input type="number" min="0" step="1" value={walletForm.available_balance} onChange={(e) => setWalletForm({ ...walletForm, available_balance: e.target.value })} className="form-control" />
                    </div>
                  </div>

                  {/* Wallet history tables */}
                  <div className="mb-3">
                    <p className="fw-bold">Earnings (credits)</p>
                    {walletEarnings.length === 0 ? (
                      <p>No earnings yet</p>
                    ) : (
                        ...
                        <tbody>
                          {walletEarnings.map((e) => (
                            <tr key={e.id}>
                              ...
                              <td className="text-end text-success">+₹{Number(e.teacher_earn ?? 0).toLocaleString()}</td>
                              <td>{e.status || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                  ...
                  <div className="mb-3">
                    <p className="fw-bold">Withdrawals</p>
                    {walletWithdrawals.length === 0 ? (
                        ...
                        <tbody>
                          {walletWithdrawals.map((w) => (
                            <tr key={w.id}>
                              ...
                              <td className="text-end text-warning">−₹{Number(w.amount ?? 0).toLocaleString()}</td>
                              <td>{w.status || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                  ...
                  <div className="d-flex justify-content-end gap-2">
                    <button type="button" onClick={() => setModalType(null)} className="btn btn-outline-light">Cancel</button>
                    <button type="button" onClick={handleSaveWallet} className="btn btn-primary" disabled={walletLoading}>Save Wallet</button>
                  </div>
                </>
              )}
            </div>
